...
DataGridDefinition getGridDefFromMap(Map<String,Map<String,List<String>>> gridMap, log=false, Map<String,Boolean> suppress=[:], Cube cube=rule.getCube()){
    ...
    * @param suppress : Suppression Options as List<boolean> in the order [SuppressMissingColumns, SuppressMissingRows, SuppressMissingRowsNative, SuppressMissingBlocks]
...
List<String> getStoredMbrs(List<String> mbrs, Dimension dim, Cube cube=rule.getCube()){
	/**
    ...
    */
    List<String> mbrReturn = []
    mbrs.each{ mbr ->
        Map mbrCur = dim.getMember(mbr).toMap()
        if(["never share","store"].contains(mbrCur["Data Storage"])) {
            mbrReturn << mbr
        }
    }
